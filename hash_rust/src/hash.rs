fn padding_bits(message: &mut Vec<u8>) -> Vec<u8> {
    ...
    let mut zero_padding = vec![0u8; zero_padding_bytes];
...
pub fn initialize_w(input_message: &[u8]) -> [u32; 64] {
    ...
    for i in 16..64 {
        ...
        let s1 = (w[i - 2] >> 17) ^ (w[i - 2] << 15) ^ (w[i - 2] >> 10);
...
pub fn compress_function(w: &mut [u32; 64]) -> [u32; 8] {
    ...
    let new_a = a.wrapping_add(a);
    let new_b = b.wrapping_add(b);
    let new_c = c.wrapping_add(c);
    let new_d = d.wrapping_add(d);
    let new_e = e.wrapping_add(e);
    let new_f = f.wrapping_add(f);
    let new_g = g.wrapping_add(g);
    let new_h = h.wrapping_add(h);
